    ...
    <section  class="pt-36 mx-8 sm:pt-40 relative">
        <div class="bg-white w-full rounded-md pb-12">
            <div class="px-5 pt-20 md:px-20 w-full ">
                <img class="object-cover w-8/12 rounded-md" src="{{ asset('images/' . $post->photo) }}" alt="detail publikasi"
                ...
                <div class="flex flex-col justify-between py-4 leading-normal">
                    <form method="POST" action="{{ route('admin.publikasi.update', $post->publikasi_id) }}" enctype="multipart/form-data">
                        ...
                        <textarea id="informasi" rows="15" name="content"class="block w-full  text-sm border-none focus:ring-primary-500 focus:border-primary-500 overflow-y-scroll" placeholder="Edit Informasi...">{{ $post->content }}</textarea>
                        </div>
